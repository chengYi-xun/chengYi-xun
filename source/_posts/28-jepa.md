...
>
> 即 $s$ 保留了所有与预测目标 $y$ 相关的信息，同时丢弃与 $y$ 无关的信息。这正好是**最小充分统计量**（Minimal Sufficient Statistics）的定义，与 Information Bottleneck（Tishby et al., 2000）一脉相承。
...
class IJEPA(nn.Module):
    """Minimal I-JEPA-style training loop (illustrative)."""

    def __init__(self, encoder, predictor, embed_dim, ema_decay=0.996):
        """Wire student encoder, EMA target copy, and predictor.

        Args:
            encoder: Context (student) encoder.
            predictor: Maps context tokens to target-token predictions.
            embed_dim: Token width (unused in this minimal sketch).
            ema_decay: Target EMA coefficient in ``[0, 1)``.
        """
        super().__init__()
    ...
    def update_target_encoder(self):
        """EMA-update target encoder from context encoder weights.
    ...
    def forward(self, images, context_masks, target_masks):
        """Compute averaged MSE between predicted and target tokens.

        Args:
            images: RGB batch, shape (B, C, H, W).
            context_masks: Visible patch indices for context encoder.
            target_masks: List of index tensors, one per target block.

        Returns:
            Scalar loss averaged over target blocks.
        """
        # Context path: only visible patches
        context_tokens = self.context_encoder(images, mask=context_masks)  # (B, N_ctx, D)

        with torch.no_grad():
            # Full-image target tokens (B, N, D)
            target_tokens = self.target_encoder(images)

        loss = 0.0
        for tgt_mask in target_masks:
            pred = self.predictor(context_tokens, tgt_mask)  # (B, N_tgt, D)
            target = target_tokens[tgt_mask]  # (B, N_tgt, D)
