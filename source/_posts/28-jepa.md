...
- **目标块较大**（论文中目标块面积比例随机采样于约 **15%–20%** 区间），迫使模型理解语义而非纹理

- **多个目标块**（通常 **$M=4$**），从不同位置预测

- **上下文块**为单个大块（面积约 **85%–100%**），并移除与目标重叠的区域，保证预测非平凡
...
| ViT-H/14 预训练 GPU hours（论文 §7） | 显著更高（相对 I-JEPA 约 **>10×**） | **<1200** GPU h |
