...
class DinoVisionTransformer(nn.Module):
    """DINOv2 ViT backbone with optional register tokens."""

    def __init__(self, ..., num_register_tokens=0, ...):
        """
        Args:
            num_register_tokens: Extra register count N; 0 disables registers.
        """
        ...
        self.num_register_tokens = num_register_tokens

        # [CLS] token 与仅覆盖 [CLS]+patch 的位置编码
        self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))  # (1, 1, D)
        self.pos_embed = nn.Parameter(
            torch.zeros(1, num_patches + 1, embed_dim)
        )  # (1, 1+M, D)

        # Register tokens: (1, N, D)
    ...
    def init_weights(self):
        """Initialize pos_embed, cls_token, and register_tokens when present."""
...
def prepare_tokens_with_masks(self, x, masks=None):
    """
    Build token sequence: [CLS], optional [REG]*, then patch tokens.

    Args:
        x: Input images, shape (B, nc, h, w).
        masks: Optional mask tensor for masked training (unused in snippet).

    Returns:
        Token tensor of shape (B, 1 + N + M, D) when registers enabled,
        else (B, 1 + M, D).
    """
    B, nc, w, h = x.shape
    x = self.patch_embed(x)  # (B, M, D)

    # [CLS] 与 patch 先拼接，再加仅针对 [CLS]+patch 的位置编码
    x = torch.cat((self.cls_token.expand(B, -1, -1), x), dim=1)  # (B, 1+M, D)
    x = x + self.interpolate_pos_encoding(x, w, h)

    # 在 [CLS] 与 patch 之间插入 register，reg 不加 pos_embed
    if self.register_tokens is not None:
        x = torch.cat(
            (
                x[:, :1],  # (B, 1, D) [CLS]
                self.register_tokens.expand(B, -1, -1),  # (B, N, D)
                x[:, 1:],  # (B, M, D) patches
            ),
            dim=1,
        )  # (B, 1+N+M, D)
...
def forward_features(self, x, masks=None):
    ...
    Args:
        x: Input images, shape (B, nc, h, w).
        masks: Optional mask for training.

    Returns:
        Dict with keys ``x_norm_clstoken`` (B, D),
        ``x_norm_regtokens`` (B, N, D), ``x_norm_patchtokens`` (B, M, D).
    """
    ...
    return {
        ...
        "x_norm_regtokens": x_norm[:, 1 : n + 1],
        "x_norm_patchtokens": x_norm[:, n + 1 :],
    }
```
...
```python
# VGGT - aggregator.py
# 为 register / camera 等特殊 token 使用零位置编码，再与 patch 位置拼接
...
# last_hidden_state 顺序：[CLS], [REG_1..4], patch_1..M
lhs = outputs.last_hidden_state  # (B, 1+4+M, D)
cls_token = lhs[:, 0]  # (B, D) 全局表征
patch_tokens = lhs[:, 5:]  # (B, M, D) 跳过 1 个 CLS 与 4 个 REG
```
...
class MyViTWithRegisters(nn.Module):
    """Minimal ViT-style module showing register-token placement."""

    def __init__(self, embed_dim, num_patches, num_registers=4):
        """
        Args:
            embed_dim: Token dimension D.
            num_patches: Patch token count M.
            num_registers: Number of register tokens N.
        """
        super().__init__()
        self.num_registers = num_registers

        # 步骤 1：可学习 [CLS]、register、仅覆盖 [CLS]+patch 的 pos_embed
    ...
    def forward(self, patch_tokens):
        """
        Args:
            patch_tokens: Patch embeddings, shape (B, M, D).

        Returns:
            Tuple ``(cls_out, patch_out)`` of shapes (B, D) and (B, M, D);
            register tokens are dropped.
        """
        B = patch_tokens.shape[0]

        # 步骤 2：先 [CLS]+patch 加位置编码，再在中间插入 register
        cls = self.cls_token.expand(B, -1, -1)  # (B, 1, D)
        x = torch.cat([cls, patch_tokens], dim=1)  # (B, 1+M, D)
        x = x + self.pos_embed

        reg = self.register_tokens.expand(B, -1, -1)  # (B, N, D)
        x = torch.cat([x[:, :1], reg, x[:, 1:]], dim=1)  # (B, 1+N+M, D)
