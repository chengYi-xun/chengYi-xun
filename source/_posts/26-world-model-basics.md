...
本文将带你从零开始，循序渐进地理解这个领域的全貌，并深入到核心的数学公式与理论推导中。
...
对于潜变量世界模型，所有路线的训练目标都可以统一写为 ELBO（证据下界）。这是变分推断的核心结果：
...
重建项迫使潜状态 $s_t$ 保留足够信息来解释观测 $o_t$；KL 项迫使推断出的状态分布与模型自身预测的状态分布保持一致。
...
2018 年，David Ha 和 Jürgen Schmidhuber 发表了论文 World Models（arXiv:1803.10122），首次将世界模型这个概念在深度学习中系统化。它的架构由三个模块组成：

V 模型（压缩感知）：使用变分自编码器（VAE）将高维观测压缩为低维潜变量 $z_t = \text{Encoder}_\phi(o_t) \in \mathbb{R}^{32}$。

M 模型（动力学预测）：使用混合密度网络（MDN-RNN），在潜在空间中预测未来。给定当前潜变量 $z_t$ 和动作 $a_t$，预测下一个潜变量 $z_{t+1}$ 的分布。M 模型使用 LSTM 维护一个隐状态 $h_t$，输出一个 MDN：
...
C 模型（控制器）：一个极简的线性控制器，直接从潜变量和隐状态输出动作 $a_t = W_c [z_t; h_t] + b_c$。

这是 World Models 论文最引人注目的实验：完全在 M 模型生成的梦境中训练控制器，然后直接迁移到真实环境。

---

# 潜在空间：为什么不在像素空间建模？

这是世界模型设计中的核心选择。一帧 $64 \times 64 \times 3$ 的图像有 12,288 维。预测下一帧就是在 12,288 维空间中建模条件分布——这在统计上极其困难。潜在空间 $z \in \mathbb{R}^{32}$ 将维度降低了 384 倍。

从信息论视角看，VAE 的潜空间就是在做有损压缩。根据率失真理论（Shannon 1959），在码率不超过 $R$ 比特的约束下，最小可达失真为 $D(R) = \min_{p(\hat{x}|x): I(X;\hat{X}) \leq R} \mathbb{E}[d(X, \hat{X})]$。一个好的世界模型应该压缩掉与预测无关的高熵细节（如纹理噪声），保留与动力学相关的语义信息（如物体位置、速度）。

在像素空间做多步预测时，误差累积是致命的。定理（复合误差上界）指出，对于 $H$ 步想象 rollout，若单步动力学模型的 Lipschitz 常数为 $L$，第 $k$ 步的单步预测误差为 $\epsilon_k$，则终态误差满足 $\|s_{t+H} - \hat{s}_{t+H}\| \leq \sum_{k=0}^{H-1} L^{H-1-k} \cdot \epsilon_k$。当 $L > 1$ 时（混沌系统），误差呈指数增长。在低维潜空间中，模型更容易学到 Lipschitz 常数较小的动力学，误差累积因此更可控。
...
Dreamer 系列（Hafner et al., 2020-2023）是 Model-based RL 中最成功的世界模型家族。其核心架构是循环状态空间模型（Recurrent State-Space Model, RSSM），巧妙地将确定性和随机性组合在一起。
...
DreamerV3（2023）的核心理论贡献是鲁棒性技巧的组合，使其能使用同一套固定超参数横扫 150 多个任务：

首先是 Symlog 变换。在强化学习中，不同环境的奖励和观测尺度可能相差数个数量级（例如 Atari 游戏得分从 1 到 100,000 不等）。DreamerV3 引入了对称对数变换 $\text{symlog}(x) = \text{sign}(x) \ln(|x|+1)$，将大数值压缩到对数尺度，同时在原点附近保持近似线性。这使得模型能够使用单一的均方误差损失来拟合极其多样的目标分布。

其次是 Two-hot 编码。传统强化学习通常使用标量回归来预测价值函数，但价值分布往往是多模态的。DreamerV3 将连续值离散化为 $K$ 个桶，将目标值映射为相邻两个桶的线性组合（Two-hot），从而将回归问题转化为分类问题。这种方法不仅提供了更丰富的梯度信号，还能自然地表达不确定性。

最后是 KL 自由位（Free Bits）与 KL 平衡。在变分推断中，强大的自回归模型往往会忽略潜变量，导致后验坍塌。DreamerV3 将 KL 散度拆分为动力学损失（优化先验）和表示损失（优化后验），并引入了 Free Bits 截断：$\mathcal{L}_{\text{dyn}} = \max(1, \text{KL}[\text{sg}(q) \| p])$。这允许模型在每个时间步保留至少 1 nat 的信息量，防止模型为了最小化 KL 散度而完全放弃对环境随机性的建模。
