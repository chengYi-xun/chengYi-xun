...
在前面的 RLHF 框架中，我们需要**先训练一个独立的奖励模型 $r_\theta$**，然后用 PPO 等算法去优化策略。但 Rafailov et al.（2023）提出的**直接偏好优化（DPO）**发现了一条捷径：在带 KL 正则的 RLHF 目标下，最优策略和奖励之间存在一个**封闭解**关系，可以把奖励 $r$ 用策略 $\pi_\theta$ 和参考策略 $\pi_{\text{ref}}$ 的比值来表达：

$$
r(x,y) = \beta\log\frac{\pi_\theta(y|x)}{\pi_{\text{ref}}(y|x)} + \text{const}
$$

把这个"隐式奖励"代回 Bradley–Terry 公式 $\sigma(r_w - r_l)$，你会发现：**我们可以跳过训练独立的 $r_\theta$，直接用策略模型本身的输出概率来做偏好学习**。这就是 DPO 的核心思想。

这里的关键要点不是说"奖励模型没用了"，而是：**BT 配对似然 $\sigma(\cdot)$ 是所有偏好建模方法的共用数学骨架**。RLHF 和 DPO 的区别仅在于：奖励是**显式**地训一个独立的 $r_\theta$（RLHF），还是**隐式**地折叠进策略模型的概率比里（DPO）。
...
对整条 $(x,y)$，仅在**最后一个 token**（EOS）处输出一个标量，语义是“整段回答的总分”。训练使用**配对对比损失**（即上文的 BT 损失），监督信号是**相对偏好**（A 比 B 好）。这是 InstructGPT 式 RM 的默认粒度，最适合开放式对话、安全性对齐等主观任务。

**2. 结果级（Outcome Reward Model, ORM）**
ORM 同样只关心**最终答案是否正确**，但与 BT-RM 有两个根本区别：

- **损失函数不同**：ORM 使用**逐 token 的二元交叉熵损失**（而非配对对比损失）。训练时，将“正确/错误”的结果标签复制到回答中的每个 token 上，模型在每个位置都学习预测这条回答是否正确。
- **监督信号不同**：ORM 的标签是**绝对的对/错**（不需要配对比较），而 BT-RM 学习的是“谁比谁好”这种相对关系。

推断时，通常取最后一个 token 的正确性概率作为整条解答的得分。

> **常见混淆**：如果你拿“正确 vs 错误”的配对去训练 BT 对比损失，那它**技术上仍然是 BT-RM**，不是 ORM。真正的 ORM 由其**逐 token 交叉熵训练目标**定义（参考 Nathan Lambert, *RLHF Book*, Ch.5; Cobbe et al., 2021）。

*局限*：ORM 难以惩罚“过程全错但最后蒙对答案”的情况（False Positives），因为结果标签对所有 token 是相同的。

**3. 过程级（Process Reward Model, PRM）**
PRM 对推理链的**每一个中间步骤**进行打分（正 / 负 / 中性）。与 ORM 相比，PRM 的标签不再是“整条回答对不对”，而是“这一步推理对不对”——训练时，每个推理步骤末尾的 token 获得该步骤自己的标签。在 OpenAI 的 *Let's Verify Step by Step* 中，PRM 被证明在复杂数学推理上显著优于 ORM，因为它能精准定位逻辑错误发生在哪一步。
...
- **主观对话、安全、风格**：必须依赖 **序列级 BT-RM**（相对偏好，无客观答案）。
- **客观可验证（代码/数学结果）**：首选 **可验证奖励**（规则判定，零成本）。当规则检查不可用或需要软概率时，退而选择 **ORM**。
- **可验证任务 + Best-of-$N$ 重排序**：**ORM** 是工程实践中的默认选择——只需最终答案标签，比 PRM 便宜得多，却比硬规则提供更细腻的评分。
- **复杂多步推理且搜索空间大**：**PRM** 最善于引导搜索和剪枝，但需要逐步标注，系统复杂度最高。
