...
前面我们推导出了完美的理论公式，但在实际写代码时，我们无法计算出真正的“期望” $\mathbb{E}$，只能通过**采样（Sampling）**来近似。根据我们如何估计公式中的优势函数 $A(s_t, a_t)$ 或动作价值 $Q(s_t, a_t)$，策略梯度算法经历了以下几个阶段的演进。

## 1. REINFORCE 算法 (Monte-Carlo Policy Gradient)

最基础的策略梯度算法是 **REINFORCE**。它的核心思想是：**用实际采样到的轨迹回报，来代替理论上的期望值 $Q$。**

在一条采样到的轨迹中，智能体在 $t$ 时刻执行动作 $a_t$ 后，实际获得的累积折扣奖励记为 $G_t$：
$$
G_t = \sum_{k=0}^{T-t} \gamma^k r_{t+k}
$$
由于 $G_t$ 是 $Q^{\pi_\theta}(s_t, a_t)$ 的一个无偏估计（Unbiased Estimate，**即虽然每次采样的 $G_t$ 波动很大，但它们求期望（平均值）后刚好等于真实的 $Q$ 值**），我们可以直接用 $G_t$ 替换梯度公式中的 $Q$。
...
**REINFORCE 的致命缺点：**
1. **方差极大**：一条轨迹的累积奖励 $G_t$ 受到整条轨迹上所有随机状态转移和随机动作的影响，波动非常剧烈。
2. **回合更新（Episodic）**：必须等一局游戏（Episode）完全结束，才能从后往前计算出所有的 $G_t$ 并更新参数，学习效率极低。

## 2. 引入基线 (REINFORCE with Baseline)

为了解决 REINFORCE 方差极大的问题，我们自然会想到前面推导过的**“基线相减”**技巧。

我们在 $G_t$ 的基础上减去一个状态价值的估计值 $V_\phi(s_t)$（通常用另一个神经网络来拟合，参数为 $\phi$）。这样，我们就用 $(G_t - V_\phi(s_t))$ 来近似优势函数 $A(s_t, a_t)$。
