...
while not done:
    ...
    action = dist.sample()
    log_prob = dist.log_prob(action)
    value = critic(state).squeeze()       # V_φ(s) — 基线估计
    next_state, reward, done = env.step(action)
...
for (log_prob, _, value), G_t in zip(trajectory, returns):
    advantage = G_t - value.detach()      # A ≈ G_t - V_φ(s), 停梯度防止影响 Critic
    policy_loss += -log_prob * advantage  # Actor: 推高好动作概率
    value_loss += F.mse_loss(value, torch.tensor(G_t))  # Critic: 让 V_φ(s) 逼近 G_t
...
for step in range(max_steps):
    ...
    action = dist.sample()
    log_prob = dist.log_prob(action)
    value = critic(state).squeeze()                 # V_φ(s)

    next_state, reward, done = env.step(action)
    next_value = critic(next_state).squeeze().detach()  # V_φ(s'), 停梯度
    ...
    # ============================================================
    actor_loss = -log_prob * delta.detach()    # Actor: 用 δ 作为优势, 调整动作概率
    critic_loss = delta.pow(2)                 # Critic: 让 V_φ(s) 逼近 TD 目标
