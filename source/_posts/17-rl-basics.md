...
> **与 on-policy / off-policy 的区别**：在线/离线说的是"训练时是否与环境交互"，而 on-policy / off-policy 说的是"用谁产生的数据来更新"。例如，PPO 是在线 + **近似** on-policy（用当前策略采集数据，通过重要性采样复用几个 epoch）；DPO 是离线 + off-policy（用其他策略收集的偏好数据训练）。
