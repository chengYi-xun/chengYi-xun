...
5. [笔记｜生成模型（五）：DDPM理论](../5-ddpm)
6. [笔记｜生成模型（六）：DDIM理论](../6-ddim)
7. [笔记｜生成模型（七）：Score-Based理论](../7-score-match)
8. [笔记｜生成模型（八）：SDE统一DDPM和SMLD](../8-SDE)
9. [笔记｜生成模型（九）：Classifier Guidance 理论与实现](../9-classifier-guidance-for-diffusion-models)
10. [笔记｜生成模型（十）：Classifier-Free Guidance 理论与实现](../10-classifier-free-guidance-for-diffusion-models)
11. [笔记｜生成模型（十一）：UIT和DiT架构详解](../11-UIT-VS-DIT)
12. [笔记｜生成模型（十二）：Normalizing Flow理论与实现](../12-Normalizing-flow)
13. [笔记｜生成模型（十三）：Flow Matching理论与实现](../13-flow_matching)
14. [笔记｜生成模型（十四）：Stable Diffusion 3 架构解析 (MMDiT)](../14-sd3)
15. [笔记｜生成模型（十五）：Flux 架构解析](../15-flux)
...
101. [笔记｜世界模型（一）：什么是世界模型？从认知科学到深度学习](../101-world-model-basics)
102. [笔记｜世界模型（二）：Dreamer 系列——在想象中学习控制](../102-dreamer)
103. [笔记｜世界模型（三）：JEPA——在嵌入空间预测世界](../103-jepa)
104. [笔记｜世界模型（四）：视频生成即世界模拟——从 Sora 到 Genie 与 Cosmos](../104-video-world-model)
105. [笔记｜世界模型（五）：物理化的视频生成——让模型理解牛顿定律](../105-physics-world-model)
106. [笔记｜世界模型（六）：自动驾驶世界模型——从视频预测到占用预测](../106-driving-world-model)
107. [笔记｜世界模型（七）：前沿与统一视角——五条路线的收敛](../107-world-model-frontier)

# 杂谈

1. [杂谈｜写作的目的](../some_think)
