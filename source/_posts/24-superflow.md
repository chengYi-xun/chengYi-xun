...
**物理代价（为什么说在烧钱？）**：
大语言模型生成 16 段文字可能只需要几秒钟。但在图像生成（Flow Matching）中，生成一张 1024×1024 的高分辨率图像，需要进行 50 步繁重的 ODE（常微分方程）或 SDE（随机微分方程）求解。*(注：为了在强化学习中实现动作的“试错”与探索，通常必须将确定性的 ODE 转化为带有随机噪声的 SDE 进行采样，关于 SDE 的详细理论基础，请参考[笔记｜生成模型（八）：SDE统一DDPM和SMLD](/chengYi-xun/posts/9-SDE/))*。
