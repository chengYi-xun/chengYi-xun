...
{% note warning no-icon %}
**为什么 clipping 失效会导致图像质量崩溃？**

PPO 的 clipping 本质上是一个**信任域（trust region）**约束：限制每次策略更新的步长不超过 $\varepsilon$。但当 ratio 存在系统性负偏差时，正样本的 ratio 在 $[1-\varepsilon, 1+\varepsilon]$ 内部"永远触不到上界"，于是 clipping 对正样本形同虚设。

这导致策略模型对高奖励样本的梯度更新**不受步长约束**，后果是：

1. **分布坍缩（mode collapse）**：策略在连续 latent 空间中**过度集中**到少数高分样本的邻域。高维空间中（$d \sim 65536$），一次过大的步长就足以将输出分布压缩到极窄的区域，生成图像的多样性急剧下降。
2. **训练振荡与发散**：无约束的大步长在高维空间中极易跨过最优区域（overshooting），下一步又被拉回，形成**策略更新的来回震荡**，训练 loss 不收敛。
3. **正负样本梯度失衡**：负样本（低分图）的 ratio 已经远小于 $1-\varepsilon$，被 clipping 截断后梯度为零——模型"忘记"了什么是坏图，只在单方向上无约束地移动，失去了 GRPO 中"组内对比"的平衡效果。

简而言之：问题不在于奖励信号的方向对不对，而在于**步子太大扯着了**——即使奖励模型完美无误，只要信任域被突破，高维连续空间中的策略也会因过大的更新步长而崩溃。
{% endnote %}
